...
const isValidXboxGameTag = (name: string): boolean => /^[a-zA-Z0-9 ]{1,15}$/.test(name);

const check = async (name: string, proxy?: IProxyConfig): Promise<boolean> => {
    const agent = getProxyAgent(proxy);

    try {
        const res = await fetch(`https://xboxgamertag.com/search/${encodeURIComponent(name)}`, {
            method: "GET",
            dispatcher: agent,
            headers: {
        ...
        return html.includes('class="profile"') || html.includes('gamertag') || !html.includes('not found');
...
export async function xbox(
    username: string,
    collection?: Map<string, any>,
    proxy?: IProxyConfig
): Promise<IResponse> {
    try {
        if (!isValidXboxGameTag(username)) {
            return {
                platform: "xbox",
                username,
                available: false,
        ...
        if (!available && collection?.get("suggestions.enabled")) {
            ...
            for (const candidate of candidates) {
                ...
                if (!isValidXboxGameTag(candidate)) continue;
